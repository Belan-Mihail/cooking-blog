...
class TestView(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username='admin', password='abc564!'
        )
    ...
    def test_cookingrecipespostlist_status_and_used_template(self):
        ...
        and used template CookingRecipesPostList
    ...
    def test_cookingrecipepostcreate_status_and_used_template(self):
        ...
        self.assertTemplateUsed(
            response, 'recipe_create.html', 'base.html'
        )
    ...
    def test_cookingrecipepostupdate_status_and_used_template(self):
        ...
        response = self.client.get(
            reverse(
                'recipe_update', kwargs={'slug': self.post.slug}
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'recipe_update.html', 'base.html'
        )
    ...
    def test_cookingrecipepostdelete_status_and_used_template(self):
        ...
        response = self.client.get(
            reverse(
                'recipe_delete', kwargs={'slug': self.post.slug}
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'recipe_delete.html', 'base.html'
        )
    ...
    def test_cookingrecipepostcategory_status_and_used_template(self):
        ...
        post = self.post
        response = self.client.get(
            reverse('selectedcategory', args=[self.post.cat.slug])
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'index.html', 'base.html')
    ...
    def test_deletemycomment_status_and_redirect(self):
        ...
        """
        response = self.client.get(
            reverse('commentdelete', args=[self.post.id])
        )
    ...
    def test_postlike_status_and_redirect(self):
        ...
        response = self.client.post(
            reverse('post_like', kwargs={'slug': self.post.slug})
            )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse(
                'cooking_recipe_post_detail', args=[self.post.slug]
            )
        )
