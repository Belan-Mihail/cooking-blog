...
class TestView(TestCase):
    ...
    def test_create_profile_used_template(self):
        ...
        self.assertTrue(
            response, 'customactions/create_profile.html' in (
                t.name for t in response.templates
            )
        )
        self.assertTrue(
            response, 'base.html' in (t.name for t in response.templates)
        )
    ...
    def test_view_profile_used_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            response, 'customactions/profile_page.html' in (
                t.name for t in response.templates
            )
        )
        self.assertTrue(
    ...
    def test_update_profile_used_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            response, 'customactions/update_profile.html' in (
                t.name for t in response.templates
            )
        )
        self.assertTrue(
    ...
    def test_contact_used_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            response, 'customactions/contact.html' in (
                t.name for t in response.templates
            )
        )
        self.assertTrue(
